...
An authorised admin can answer a conversation from `/app/inbox`. It is off by
default (`INBOX_SMTP_ENABLED` unset) and switched on separately from the
importer — reading mail and sending mail are different risks.

```dotenv
INBOX_SMTP_ENABLED=true
INBOX_SMTP_HOST=smtp.hostinger.com
INBOX_SMTP_PORT=465
INBOX_SMTP_SECURE=true
INBOX_SMTP_USER=info@peptideeinkaufen.de
INBOX_SMTP_PASSWORD=          # the real password, only in the env file
INBOX_SMTP_FROM=info@peptideeinkaufen.de
```

What a reply is, exactly:

- **plain text only**, from `info@peptideeinkaufen.de`, to the newest inbound
  message's `Reply-To` (falling back to its `From`);
- threaded with `In-Reply-To`, `References` and a subject that gains `Re:` only
  if it does not already carry a reply prefix (`Re:`, `AW:`, `WG:`, …);
- stored in the conversation with its own generated `Message-ID` and a delivery
  status of `pending`, `sent` or `failed`.

What it is not: there is no recipient field, no cc, no bcc, no subject field,
no attachment, no HTML, no forward, no template and no bulk send. The endpoint
accepts a body and an idempotency key, and nothing else — see
`src/api/admin/inbox/threads/[id]/reply/route.ts`.

### ⚠️ Sent replies are not in the mailbox's Sent folder

**This release adds no IMAP write access.** Nothing appends a copy of an
outgoing reply to the mailbox, so a reply sent from `/app/inbox`:

- **is** stored in Medusa and visible in the conversation there;
- **is** delivered to the customer normally;
- **is not** visible in Hostinger Webmail's Sent folder;
- **is not** visible to anyone reading the mailbox in a mail client.

Two consequences worth knowing before you rely on it. Someone working only in
webmail cannot see that an answer was already sent, so a thread can be answered
twice by two people. And the customer's own reply comes back to `info@` as
normal, so the conversation continues correctly on the inbound side either way.

Appending to the Sent folder needs IMAP write access, which is deliberately out
of scope here — the mailbox stays read-only in this release.

### One reply per click

Every send carries an idempotency key generated by the composer and kept for
the whole draft. The database holds a unique index on it, and the sync-style
lock is taken on the key for the duration of the send, so:

- a double click sends one email and returns the first result;
- a retry after a *failure* reuses the same row rather than adding a second
  message to the conversation;
- a request that arrives while the first is still in flight is refused with
  `INBOX_REPLY_IN_PROGRESS` rather than queued.

Rate limits are fixed in code: one reply per thread per 10 seconds, 30 per hour
across all threads. They bound a stuck client or a stolen session, not a human
typing.

### When a reply fails

The row is written as `pending` **before** the send and moved to `sent` or
`failed` after it, so a failure is never displayed as a success and a process
killed mid-send leaves a visible pending record. The stored reason is one of
`auth`, `tls`, `rejected`, `unreachable`, `temporary` — never the SMTP server's
own sentence, which names the host and the account.

`auth` and `tls` are server-side fixes (the password, the certificate);
`rejected` means the recipient's server refused it, and retrying will not help;
`unreachable` and `temporary` are worth pressing Try again.
...
- **No forwarding, composing or bulk sending.** Replying to an existing
  conversation is the only way mail leaves this system, and the recipient is
  always one the customer supplied — see § Replying.
- **No writing to the mailbox at all.** No flag change, no move, no delete, no
  append, and no Sent-folder copy. The mailbox is opened read-only and the IMAP
  session interface has no method that could do otherwise.
...
| `src/lib/inbox/smtp.ts` | Nodemailer wrapper — one recipient, text only |
| `src/lib/inbox/reply.ts` | one reply: validation, threading, idempotency, status |
...
| `src/api/admin/inbox/**` | the seven admin routes, including the reply endpoint |
