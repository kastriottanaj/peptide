...
Re-gating is a deliberate change to [deploy/Caddyfile](deploy/Caddyfile) — restoring
a `basic_auth` block and fresh credentials — plus the test updates that go with it.
It is not a variable to flip in `caddy.env`.
